...
def create_diataxi():
    try:
        ...
        try:
            legal_act = NomikiPraxi.objects.get(
                legalActCode=data['legalActCode'])
        except NomikiPraxi.DoesNotExist:
            return Response(json.dumps({
                "error":
                f"Δεν βρέθηκε νομική πράξη με κωδικό {data['legalActCode']}"
            }),
        ...
        diataxi.save()
        return Response(diataxi.to_json(),
                        mimetype="application/json",
                        status=201)
...
def list_all_diataxi():
    ...
    return Response(diataxeis.to_json(),
                    mimetype="application/json",
                    status=200)
...
def get_diataxi(code: str):
    try:
        ...
        return Response(json.dumps(
            {"error": f"Δεν βρέθηκε διάταξη με τον κωδικό {code}"}),
                        mimetype="application/json",
                        status=404)
...
def update_diataxi(code: str):
    ...
    if nomiki_praxi_code:
        try:
            legal_act = NomikiPraxi.objects.get(
                legalActCode=nomiki_praxi_code)
        except NomikiPraxi.DoesNotExist:
            return Response(json.dumps({
                "error":
                f"Δεν βρέθηκε νομική πράξη με κωδικό {nomiki_praxi_code}"
            }),
                            mimetype="application/json",
                            status=404)
    try:
        ...
        return Response(json.dumps(
            {"error": f"Δεν βρεθηκε διάταξη με τον κωδικό: {code}"}),
                        mimetype="application/json",
                        status=404)
    except Exception as e:
        return Response(json.dumps({"error": str(e)}),
...
def delete_diataxi(code: str):
    try:
        ...
        diataxi.delete()
        return Response(json.dumps(
            {"success": f"Η διάταξη με τον κωδικό {code} διαγράφηκε"}),
                        mimetype="application/json",
                        status=200)
    except Diataxi.DoesNotExist:
        return Response(json.dumps(
