    ...
    try:
        ...
        try: 
            ...
            return Response(json.dumps({
                    "error": f"Δεν βρέθηκε μονάδα με κωδικό {data['unitCode']}"}),
        ...
        for diataxi_code in data['diataxisCodes']:
            try:
                ...
                return Response(json.dumps({
                    "error": f"Δεν βρέθηκε νομική διάταξη με κωδικό {diataxi_code}"}),
                    mimetype="application/json",
                    status=404
                )
        # create remit code
...
def update_remit(remitCode: str):
    ...
    if any(field in update_fields for field in immutable_fields):
        return Response(json.dumps({
            "error":
            "Μη επιτρεπτή ενημέρωση πεδίων: remitCode, creationDate, userCode"
        }),
                        mimetype="application/json",
                        status=400)
    ...
    if unit_code:
        try:
            ...
            return Response(json.dumps({
                "error":
                f"Δεν βρέθηκε Μονάδα με κωδικό {unit_code}"
            }),
                            mimetype="application/json",
                            status=404)
    ...
    if diataxis_codes:
        for code in diataxis_codes:
            try: 
                ...
                return Response(json.dumps({
                    "error": f"Δεν βρέθηκε νομική διάταξη με κωδικό {code}"}),
                    mimetype="application/json",
                    status=404
                )
...
def delete_remit(remitCode: str):
    try:
        ...
        remit.detele()
        return Response(json.dumps(
            {"success": f"Η αρμοδιότητα με τον κωδικό {remitCode} διαγράφηκε"}),
                        mimetype="application/json",
                        status=200)
    except Remit.DoesNotExist:
        return Response(json.dumps(
            {"error": f"Δεν βρέθηκε αρμοδιότητα με τον κωδικό: {remitCode}"}),
                        mimetype="application/json",
                        status=404)
